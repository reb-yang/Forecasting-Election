...
nocite: | 
  ...
  @broom.mixed,  @gridExtra, @ggpubr, @markdown1, @markdown2, @markdown3
---
...
source(here("scripts/auxiliary_data.R"), local = knitr::knit_global())  # loading in data from scripts
source(here("scripts/data_cleaning_survey.R"), local = knitr::knit_global())
source(here("scripts/data_cleaning_post_strat.R"), local = knitr::knit_global())
...
In the sampling for the model, there was one divergent transition. Divergences can be an indication that the model may not be a good fit for the data, however this was only occurred in 1 out of 4000 samples, so it is not any cause for concern^[I used `adapt_delta` = 0.99, but it has also been suggested to change `step_size` and `max_treedepth` from the defaults]. 
